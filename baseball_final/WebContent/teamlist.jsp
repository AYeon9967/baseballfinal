...
<%
	...
	List<Post> list = null;
	if(desc != null) {
		list = ps.postlist(team); // 오래된순(오름차순)
	} else {
		list = ps.postlistdesc(team); // 최신순(내림차순)
	}
...
<div class="container">
	...
	<div id="desc">
    	<table id="desctable">
    		<tr>
				<td><a href="teamlist.do?team=<%= team %>">최신순</a></td>
				<td><a href="teamlist.do?team=<%= team %>&desc=desc">오래된순</a></td>
			</tr>
    	</table>
	</div>
		...
		<table id="posttable">
			<% for (Post p : list) { %>
				<tr>
					...
					<td><%= p.getWriternick() %>(<%= p.getWriter() %>)</td>
					<td><%= p.getUploaddate() %></td>
				</tr>
			<% postcount++; }
			if (postcount == 0) { %> <tr><td>작성된 게시글이 없습니다.</td></tr> <% } %>
		</table>
	</div>
